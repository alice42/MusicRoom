...
export const updatePrivacyMethod = async ({ token, privacyValue, dataType }) => {
  ...
  try {
    const response = await basicFetch(
      ...
      url,
      {
        headers: { 'X-SessionID': token }
      },
      { privacyValue, dataType }
    )
    return response
  } catch (err) {
    throw err
...
export const searchTracksMethod = async query => {
  const url = `${apiUrl}${api}/deezer/search?query=${query}`
  try {
...
export const linkAccountMethod = async ({ token, type, key }) => {
...
export const unlinkAccountMethod = async ({ token, type, key }) => {
