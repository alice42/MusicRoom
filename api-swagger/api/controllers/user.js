...
const createHash = () => [...Array(36)].map(() => Math.random().toString(36)[3]).join('')
...
async function recover(req, res) {
  try {
    ...
    return res.status(200).send({
      message: 'if there is an account with this email, you will receive a mail to reconnect you'
    })
  } catch (err) {
...
async function newPassword(req, res) {
  try {
    ...
    if (user) {
      ...
      await sendEmail(mailNewPassword({ email, password: newPassword }), res.mail)
    }
    return res
      .status(200)
      .send({ message: 'Password reset. A new password was send to your mail.' })
  } catch (err) {
    console.log('INTER ERROR', err.message)
    return res.status(500).send({ error: 'internal server error' })
  }
...
async function signIn(req, res) {
  try {
    ...
    return res.status(200).send({ message: 'account created, waiting for mail confirmation' })
  } catch (err) {
    console.log('INTER ERROR', err.message)
    return res.status(500).send({ error: 'internal server error' })
  }
...
async function updatePrivacy(req, res) {
  try {
    const database = res.database
    const allowedKey = ['email', 'name', 'firstname', 'tags', 'avatarUri', 'networks']
    ...
    await updatetUserNode(
      _id,
      'privacy',
      { [dataType]: privacyValue === 'tags' ? privacyValue.split(',') : privacyValue },
      database
    )
    const user = await findUserBy('_id', id, database)
    return res.status(200).send(getProfileData(user))
...
async function linkAccount(req, res) {
  try {
    ...
    if (allowedKey.indexOf(type) === -1) {
      return res.status(500).send({ error: 'you cant change this kind of account' })
    }
    const sessions = await getSessions(database)
    const id = findKey(sessions, sessionToken => sessionToken === token)
    if (!id) {
