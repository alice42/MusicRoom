...
const getTracksWithVotes = (tracks, votes) => {
  ...
  Object.entries(votes).forEach(([id, value]) => {
    ...
    votesByTrack[trackId] = votesByTrack[trackId] ? votesByTrack[trackId] + value : value
  })
...
const getEventData = (id, idCorrespondance) => event => {
  return {
    ...
    allowedUsers: (event.allowedUsers || []).map(id => idCorrespondance[id].email),
    restriction: {
      endDate: Number(event.restriction.endDate),
      isRestricted: event.restriction.isRestricted === 'true',
      startDate: Number(event.restriction.startDate),
      maxDistance: Number(event.restriction.maxDistance),
      location: event.restriction.location
    }
  }
}
...
async function getEvents(req, res) {
  try {
    ...
    const { 'X-SessionID': token, location } = getValuesFromParams(req.swagger.params)
    if (!location) {
      return res.status(500).send({ error: 'you must give your location to use this feature' })
    }
    ...
    return res.status(200).send(getEventsAvailable(events, location, id, idCorrespondance))
  } catch (err) {
...
async function deleteEventMTV(req, res) {
  try {
    ...
    if (owner !== id) {
      return res.status(500).send({ error: 'you are not authorized to delete this event' })
    }
    const { _id, token: userTokens } = await findUserBy('_id', id, database)
    const validToken = await isDeezerTokenValid(userTokens.deezer)
    if (!validToken) {
...
async function updateData(req, res) {
  try {
    ...
    if (owner !== id) {
      return res.status(500).send({ error: 'you are not authorized to edit this event' })
    }
...
async function getTracks(req, res) {
  try {
    ...
    return res.status(200).send(getTracksWithVotes(tracks.tracks.data, ev.votes))
  } catch (err) {
    console.log('INTER ERROR', err.message)
    return res.status(500).send({ error: 'internal server error' })
  }
...
async function addTrack(req, res) {
  try {
    ...
    const tracks = await getPlaylistTracks(playlistId, userTokens.deezer)
    const { votes } = await findEventBy(database, 'playlistId', Number(playlistId))
...
async function voteTrack(req, res) {
  try {
    ...
    const tracks = await getPlaylistTracks(playlistId, userTokens.deezer)
    return res.status(200).send(getTracksWithVotes(tracks.tracks.data, newVotes))
  } catch (err) {
    console.log('INTER ERROR', err.message)
    return res.status(500).send({ error: 'internal server error' })
  }
